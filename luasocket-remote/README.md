...
├── MCP_Server/                   # Original MCP server (Named Pipe)
├── AI_Context/                   # Original documentation
├── luasocket-remote/             # THIS PROJECT — LuaSocket TCP additions
...
2. `File` → `Execute Script` → select `luasocket-remote/ce_mcp_bridge_remote.lua` → `Execute`
...
Point your MCP-compatible AI client (Claude Code, Codex, etc.) at `luasocket-remote/mcp_cheatengine_remote.py`.
...
See `luasocket-remote/luasocket-ce-deploy/README.md` → Building from Source. Requires Visual Studio with C++ workload and matching Lua source code.
