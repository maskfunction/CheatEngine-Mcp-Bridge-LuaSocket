-- ============================================================================
-- CHEATENGINE MCP BRIDGE v12.0.0 (LUASOCKET TCP EDITION)
-- ============================================================================

-- TCP Configuration
local TCP_PORT = 9999
local TCP_BIND_ADDR = "0.0.0.0"  -- Bind to all IPv4 interfaces
local VERSION = "12.0.0"

-- Load LuaSocket
local socket_lib_ok, socket = pcall(require, "socket")
if not socket_lib_ok then
    socket_lib_ok, socket = pcall(require, "socket.core")
end
if not socket_lib_ok then
    error("[MCP LuaSocket] LuaSocket is not installed.\n\n"
          .. "Cheat Engine does not include LuaSocket by default.\n"
          .. "Run: python deploy.py --ce \"<Cheat Engine folder>\"\n"
          .. "See luasocket-ce-deploy/README.md for details.")
end
local mime = nil
pcall(function() mime = require("mime") end)
...
local serverState = {
    ...
    timer = nil,
    connected = false,
    workerSocket = nil,     -- TCP server socket (for cross-thread close)
    clientSocket = nil,     -- TCP client socket (for cross-thread close)
    scan_memscan = nil,
    scan_foundlist = nil,
    breakpoints = {},
    breakpoint_hits = {},
...
-- ============================================================================
-- THREAD-BASED TCP SERVER
-- ============================================================================
-- I/O happens in Worker Thread. Execution happens in Main Thread.
-- Wire protocol: 4-byte LE length prefix + UTF-8 JSON-RPC body.

local function TCPWorker(thread)
    log("TCP Worker Thread Started")

    -- Use tcp4() — tcp() may bind IPv6-only on some Windows builds.
    local server = socket.tcp4()
    server:setoption("reuseaddr", true)
    local ok, bind_err = server:bind(TCP_BIND_ADDR, TCP_PORT)
    if not ok then
        log("Fatal: Failed to bind TCP " .. TCP_BIND_ADDR .. ":" .. TCP_PORT .. " — " .. tostring(bind_err))
        return
    end
    server:listen(1)
    server:settimeout(0.5)  -- Periodic timeout to check thread.Terminated

    -- For cross-thread close from StopMCPBridge
    serverState.workerSocket = server

    log("TCP Server listening on " .. TCP_BIND_ADDR .. ":" .. TCP_PORT)
    ...
    while not thread.Terminated do
        ...
        if client then
            ...
            serverState.clientSocket = client

            -- 60s read timeout — if the network drops, the loop exits and
            -- the outer loop resumes accepting new connections.
            client:settimeout(60)

            while not thread.Terminated do
                local header, read_err = client:receive(4)
                if not header then
                    break
                end

                local len = string.byte(header, 1)
                          + string.byte(header, 2) * 256
                          + string.byte(header, 3) * 65536
                          + string.byte(header, 4) * 16777216

                if len > 0 and len < 32 * 1024 * 1024 then
                    local payload, body_err = client:receive(len)
                    if payload then
                        ...
                        if response then
                            ...
                            local pcall_ok, send_ret = pcall(function()
                                return client:send(respHeader .. response)
                            end)
                            if not pcall_ok or send_ret == nil then
                                break
                            end
                        end
                    else
                    ...
                    log("TCP: Invalid payload length " .. tostring(len))
                    break
                end
            end

            serverState.connected = false
            if serverState.clientSocket == client then
                serverState.clientSocket = nil
            end
            pcall(function() client:close() end)
            log("Remote Client Disconnected")
        elseif accept_err == "timeout" then
            -- Periodic timeout to check thread.Terminated
        else
            if not thread.Terminated then
                log("TCP accept error: " .. tostring(accept_err))
            end
            sleep(100)
        end
    end

    if serverState.workerSocket == server then
        serverState.workerSocket = nil
    end
    pcall(function() server:close() end)

    log("TCP Worker Thread Terminated")
end
...
function StopMCPBridge()
    if serverState.workerThread then
        ...
        -- Force close sockets to unblock accept/read calls in the worker thread
        -- Close client socket first (breaks read loop), then server socket (breaks accept loop)
        if serverState.clientSocket then
            pcall(function() serverState.clientSocket:close() end)
            serverState.clientSocket = nil
        end
        if serverState.workerSocket then
            pcall(function() serverState.workerSocket:close() end)
            serverState.workerSocket = nil
        end
...
function StartMCPBridge()
    ...
    -- Update Global State
    log("Starting MCP Bridge v" .. VERSION .. " (LuaSocket TCP Edition)")

    serverState.running = true
    serverState.connected = false

    -- Create the Worker Thread (TCP version)
    serverState.workerThread = createThread(TCPWorker)

    log("===========================================")
    log("MCP Server Listening on TCP: 0.0.0.0:" .. TCP_PORT)
    log("Architecture: Threaded TCP I/O + Synchronized Execution")
    log("Cleanup: Zombie Prevention Active")
    log("WARNING: Ensure firewall allows inbound TCP " .. TCP_PORT)
    log("Connect from: mcp_cheatengine_remote.py --host <server_ip>")
    log("===========================================")
end
