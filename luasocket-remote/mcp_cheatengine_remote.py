...
try:
    ...
    print(f"[MCP CE LuaSocket] Import Error: {e}", file=sys.stderr, flush=True)
    sys.exit(1)
...
def debug_log(msg):
    print(f"[MCP CE LuaSocket] {msg}", file=sys.stderr, flush=True)
...
class CERemoteClient:
    """TCP client that connects to the ce_mcp_bridge_remote.lua server."""
    ...
    def connect(self):
        """Attempts to connect to the remote CE TCP Bridge."""
        try:
            self.sock = socket.create_connection((self.host, self.port), timeout=5)
            # Disable Nagle's algorithm for lower latency on small JSON-RPC messages
            self.sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
            return True
        except (ConnectionRefusedError, socket.timeout, OSError) as e:
            # debug_log(f"TCP Connect failed: {e}")
            return False

    def _recv_exact(self, n):
        """Receive exactly n bytes from the socket, handling partial reads."""
        data = b''
        while len(data) < n:
            try:
                chunk = self.sock.recv(n - len(data))
            except socket.timeout:
                raise ConnectionError("TCP read timed out")
            if not chunk:
    ...
    def _exchange_once(self, req_json):
        """Send one framed request and parse one framed response over TCP."""
        header = struct.pack('<I', len(req_json))
        self.sock.sendall(header)
        self.sock.sendall(req_json)

        resp_header_buffer = self._recv_exact(4)
        ...
        resp_body_buffer = self._recv_exact(resp_len)
        try:
            ...
            if not self.sock:
                if not self.connect():
                    raise ConnectionError(
                        f"Cheat Engine Bridge is not reachable at {self.host}:{self.port}. "
                        f"Ensure ce_mcp_bridge_remote.lua is loaded in CE and listening."
                    )
...
ce_client = None  # Initialized in main() with CLI args

# ============================================================================
# MCP SERVER - LUASOCKET TCP VERSION
# ============================================================================
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Cheat Engine MCP Bridge (LuaSocket TCP Server)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python mcp_cheatengine_remote.py
  python mcp_cheatengine_remote.py --host 192.168.1.100 --port 9999
  python mcp_cheatengine_remote.py --port 8888
        """,
    )
    parser.add_argument("--host", default="127.0.0.1", help="CE Bridge server IP or hostname (default: 127.0.0.1)")
    parser.add_argument("--port", type=int, default=9999, help="CE Bridge server TCP port (default: 9999)")
    args = parser.parse_args()

    ce_client = CERemoteClient(host=args.host, port=args.port)

    try:
        debug_log(f"Cheat Engine MCP Bridge (LuaSocket TCP) v12.0.0")
        debug_log(f"Target: {args.host}:{args.port}")
        mcp.run()
